...
import { addAmountToSaving, getSavingsByUserId , getTotalSaving} from '../models/Savings/Savings';
...
export const addGoal = async (req: RequestExtended, res: Response) => {
    try {
        ...
        const { targetAmount, monthly, startDate, endDate, title, currentAmount = 0 } = req.body;

        if (monthly === undefined || targetAmount === undefined || startDate === undefined || endDate === undefined) {
            return res.status(400).json({ message: 'Missing required fields' });
        }

        if (!monthly && !title) {
            return res.status(400).json({ message: 'Title is required for non-monthly savings' });
        }
        ...
        const duplicateSaving = existingSavings.find(saving =>
            saving.startDate.getTime() === parsedStartDate.getTime() &&
            saving.endDate.getTime() === parsedEndDate.getTime()
        );

        if (duplicateSaving) {
            return res.status(400).json({ message: 'Saving already exists for the given date' });
        }
...
export const addAmount = async (req: RequestExtended, res: Response) => {
    try {
        ...
        return res.status(500).json({ message: 'Error adding amount to saving goal' });
    }
}
